...
export function SourcesLedger({ sources }: { sources: SourceStatus[] }) {
  ...
  return (
    <main className="mx-auto w-full max-w-[1450px] pb-12">
      ...
      <section className="mt-8 border border-border bg-card"><header className="flex items-center justify-between border-b border-border p-5"><div className="flex items-center gap-3"><DatabaseZap className="size-4 text-brand" /><div><h2 className="text-sm font-semibold">Coleta automática</h2><p className="text-xs text-muted-foreground">Feeds e APIs públicas presentes no último snapshot.</p></div></div><span className="inline-flex items-center gap-2 text-[10px] uppercase tracking-wider text-muted-foreground"><Activity className="size-3 text-match-strong" />snapshot local</span></header><div className="divide-y divide-border/60">{automatic.map((source) => <SourceRow key={source.id} source={source} />)}</div></section>
...
function SourceRow({ source }: { source: SourceStatus }) {
  ...
  return <article className="grid gap-4 p-5 md:grid-cols-[48px_1fr_auto] md:items-center"><div className={cn('flex size-10 items-center justify-center border font-mono text-[10px] font-semibold uppercase', assisted ? 'border-brand/25 bg-brand/[.06] text-brand' : 'border-match-strong/25 bg-match-strong/5 text-match-strong-foreground')}>{source.label.slice(0,2)}</div><div><div className="flex items-center gap-2"><h3 className="text-sm font-semibold">{source.label}</h3>{source.status === 'ok' && <CircleCheck className="size-3.5 text-match-strong" />}</div><p className="mt-1 text-xs leading-5 text-muted-foreground">{NOTES[source.id] ?? (assisted ? 'Fonte assistida com abertura na plataforma original.' : 'Adaptador público normalizado pelo coletor.')}</p></div><div className="flex items-center gap-5 md:text-right"><div><p className="font-mono text-lg font-semibold">{assisted ? '—' : source.count}</p><p className="text-[9px] uppercase tracking-wider text-muted-foreground">{assisted ? 'assistida' : 'coletadas'}</p></div>{href && <a aria-label={`Abrir ${source.label}`} className="flex size-10 items-center justify-center border border-border text-muted-foreground transition hover:bg-muted hover:text-foreground" href={href} rel="noreferrer" target="_blank"><ExternalLink className="size-3.5" /></a>}</div></article>
}
